...
RequestHandler.prototype.parseBody = function (data, index = 0) {
    const detectByContentLength = () => {
        if (this.headers["content-length"] - 0 === this.numberOfReceivedBody) {
            console.log("body接收完毕");
            console.log(Buffer.concat(this.rawBody).toString());
            this.response();
        } else {
    ...
    console.log(`接收到的body长度是${data.length-index}，总长度是：${this.headers["content-length"]}，还剩余的长度是${this.headers["content-length"]-0-this.numberOfReceivedBody}`);
